...
function main() {
  ...
  // obstacles (sizes match the gameplay hitboxes in streetGen.ts)
  prop('ob-cone', 0.55, 0.78, 'cross', (W, H) => poly([[W / 2 - 6, 4], [W / 2 + 6, 4], [W - 12, H - 10], [12, H - 10]], '#f07b3f', 3.5) + poly([[W * 0.34, H * 0.42], [W * 0.66, H * 0.42], [W * 0.72, H * 0.6], [W * 0.28, H * 0.6]], '#f4f3ea', 2) + rect(0, H - 12, W, 10, '#3b3f3e', 2))
  prop('ob-cones', 2.6, 0.8, 'card', (W, H) => {
    let s = rect(20, 22, W - 40, 12, '#f2c230', 2.5)
    for (const cx of [30, W / 2, W - 30]) s += poly([[cx - 5, 4], [cx + 5, 4], [cx + 24, H - 10], [cx - 24, H - 10]], '#f07b3f', 3) + rect(cx - 27, H - 12, 54, 10, '#3b3f3e', 2)
    return s
  })
  prop('ob-barrier', 2.4, 0.95, 'card', (W, H) => {
    let s = stroke([[30, 30], [10, H - 4]], 5) + stroke([[30, 30], [50, H - 4]], 5) + stroke([[W - 30, 30], [W - 50, H - 4]], 5) + stroke([[W - 30, 30], [W - 10, H - 4]], 5)
    s += rect(0, 10, W, 34, '#f2c230', 3)
    for (let x = -20; x < W; x += 40) s += `<path d="M${x},44 L${x + 20},44 L${x + 40},10 L${x + 20},10Z" fill="#26282b"/>`
    return s
  })
  prop('ob-boxes', 1.1, 0.75, 'cross', (W, H) => rect(5, H * 0.38, W - 10, H * 0.6, '#c9a36f', 3.5) + seg(W / 2, H * 0.38, W / 2, H, 2) + rect(W * 0.2, 4, W * 0.55, H * 0.36, '#d8b680', 3) + shade(5, H * 0.38, W - 10, 8))
  prop('ob-bags', 1.2, 0.6, 'cross', (W, H) => `<ellipse cx="${W * 0.3}" cy="${H * 0.6}" rx="${W * 0.28}" ry="${H * 0.38}" fill="#6f9fc4" stroke="${INK}" stroke-width="3.5"/><ellipse cx="${W * 0.68}" cy="${H * 0.58}" rx="${W * 0.26}" ry="${H * 0.4}" fill="#86b3d1" stroke="${INK}" stroke-width="3.5"/>` + stroke([[W * 0.3, H * 0.24], [W * 0.26, H * 0.08], [W * 0.36, H * 0.12]], 3))
